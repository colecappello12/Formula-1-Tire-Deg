...
Now we create a new variable 'Time_Secs' which is the number of seconds since the start of the race.  We also select only observations whose source is from the car
